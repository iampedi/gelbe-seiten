...
interface AngeboteComponentProps {
  angebote: {
    title: string;
    description: string;
    icon: string;
...
export default function AngebotesComponent({
  angebote,
}: AngeboteComponentProps) {
  return (
    <div className="flex gap-4">
      {angebote.map(({ title, description, icon }) => (
        <Card
          key={title}
          className="group bg-secondary/20 border-secondary hover:bg-secondary cursor-pointer rounded-md border-2 shadow-none duration-300"
        >
          <CardHeader>
            <span
          ...
          <CardContent>
            <h4 className="mb-2 flex items-center gap-1 text-lg font-semibold">
